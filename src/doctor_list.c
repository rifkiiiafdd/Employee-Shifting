...
#include <glib.h> // Required for GString (used in get_blacklisted_preferences_string)
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
char *get_blacklisted_preferences_string(Doctor *doctor) {
    ...
    for (int i = 0; i < NUM_DAYS_PER_WEEK; i++) {
        for (int j = 0; j < NUM_SHIFTS_PER_DAY; j++) {
            if (doctor->preference[i][j] == 0) { // If blacklisted
                if (!first_pref) {
                    g_string_append(str_builder, ", ");
                }
                g_string_append_printf(str_builder, "%s %s", days[i],
                                       shifts[j]);
                first_pref = false;
            }
        }
    ...
    if (first_pref) { // No blacklisted preferences found
        g_string_append(str_builder, "None");
    }

    return g_string_free(str_builder,
                         FALSE); // Return owned string, don't free buffer
}
